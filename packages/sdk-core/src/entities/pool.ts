...
export class Pool {
  ...
  public getOutputAmount (inputAmount: TokenAmount): [TokenAmount, Pool] {
    ...
    return [
      outputAmount,
      new Pool(this.liquidityAmount, inputReserve.add(inputAmount), outputReserve.subtract(outputAmount))
    ];
  }
...
export type AbstractPair = Pick<Pool, 'token0' | 'token1' | 'reserve0' | 'reserve1' | 'getOutputAmount' | 'pathOf'>
export type AbstractPool = Pick<Pool, 'token0' | 'token1' | 'reserve0' | 'reserve1' | 'getOutputAmount' | 'pathOf'>

export { Pool as StandardPool };
